...
def llm_query_parser(query: str) -> Tuple[List[str], str]:
    ...
    try:
        ...
        class ExtractedGeoProblem(BaseModel):
            ...
            goal_fact: str = Field(
                ...
                description=(
                    ...
                    "For COMPUTE problems (tính, tìm, calculate): use 'Compute(Length(AB))' or "
                    "'Compute(Angle(ABC))' when the value is unknown. "
                    "If the answer IS known in the problem, use 'Equal(Length(AB),5)'."
                ),
            )
        ...
        system_prompt = (
            ...
            "  *** CRITICAL: 'right angle at C' in triangle ABC → RightAngle(Angle(ACB)) where C is vertex ***\n"
            "- Triangle: Triangle(A,B,C)\n"
            "- Right Triangle: RightTriangle(A,B,C) + RightAngle(Angle(BAC)) for right angle at A\n"
            "                  RightTriangle(A,B,C) + RightAngle(Angle(ACB)) for right angle at C\n"
            ...
            "- Equality of angles: Equal(Angle(ABC),Angle(DEF)) or Equal(Angle(ACB),50)\n"
            "- Equality of lengths: Equal(Length(AB),5) — segment AB has length 5\n"
            ...
            "- Circumcircle: Circumcircle(O,A,B,C) — O is circumcircle center of triangle ABC\n\n"
            "=== GOAL TYPE RULES ===\n"
            "'PROVE / CHỨNG MINH' problems: goal is the statement to prove.\n"
            "  Example: goal: 'Equal(Angle(ACB),50)'\n\n"
            "'COMPUTE / TÍNH / TÌM' problems: goal is 'Compute(quantity)' when the answer is unknown.\n"
            "  Example: 'tính cạnh huyền AB' → goal: 'Compute(Length(AB))'\n"
            "  Example: 'tính góc C' → goal: 'Compute(Angle(ACB))'\n"
            "  ONLY use Equal(Length(AB),5) when the answer is explicitly stated in the problem.\n\n"
            ...
            "'Cho tam giác ABC. Biết góc A = 60, góc B = 70. Tính góc C'\n"
            "→ facts: ['Triangle(A,B,C)', 'Equal(Angle(BAC),60)', 'Equal(Angle(ABC),70)'], goal: 'Compute(Angle(ACB))'\n\n"
            "'Cho tam giác ABC vuông tại C, AC=3, BC=4. Tính cạnh AB'\n"
            "→ facts: ['RightTriangle(A,B,C)', 'RightAngle(Angle(ACB))', 'Equal(Length(AC),3)', 'Equal(Length(BC),4)'], goal: 'Compute(Length(AB))'\n\n"
            "'Tam giác ABC vuông tại C, cạnh huyền AB = 5, AC = 3. Tính BC'\n"
            "→ facts: ['RightTriangle(A,B,C)', 'RightAngle(Angle(ACB))', 'Equal(Length(AB),5)', 'Equal(Length(AC),3)'], goal: 'Compute(Length(BC))'\n\n"
            ...
            "→ facts: ['RightTriangle(A,B,C)', 'RightAngle(Angle(BAC))'], goal: 'Equal(Pow(Length(BC),2),Add(Pow(Length(AB),2),Pow(Length(AC),2)))'\n\n"
            ...
            "→ facts: ['Circle(O)', 'PointOnCircle(A,Circle(O))', 'PointOnCircle(B,Circle(O))', 'PointOnCircle(C,Circle(O))', 'PointOnCircle(D,Circle(O))', 'IntersectionPoint(P,AB,CD)'], goal: 'Equal(Mul(Length(AP),Length(PB)),Mul(Length(CP),Length(PD)))'\n\n"
            "'Cho tứ giác ABCD là hình thoi. CM AC vuông góc BD'\n"
            "→ facts: ['Rhombus(A,B,C,D)'], goal: 'Perpendicular(AC,BD)'\n\n"
            "'Cho tứ giác nội tiếp ABCD. CM góc DAB + góc BCD = 180'\n"
            "→ facts: ['CyclicQuadrilateral(A,B,C,D)'], goal: 'Equal(Add(Angle(DAB),Angle(BCD)),180)'\n\n"
            ...
            "'E là trung điểm AB, F là trung điểm AC trong tam giác ABC. CM EF song song BC'\n"
            "→ facts: ['Triangle(A,B,C)', 'Midpoint(E,AB)', 'Midpoint(F,AC)'], goal: 'Parallel(EF,BC)'\n\n"
