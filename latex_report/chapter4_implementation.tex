...
\begin{tikzpicture}[node distance=2.5cm, auto]
    \node [base, fill=teal!15, text width=3.8cm] (frontend) {\textbf{Frontend UI}\\\faDesktop\ Streamlit (Port 8501)\\Giao diện giải toán \& Stream};
    \node [base, fill=blue!15, right of=frontend, xshift=3.5cm, text width=4.5cm] (backend) {\textbf{Backend API Gateway}\\\faServer\ FastAPI (Port 8080)\\Engine Suy diễn + SymPy AR};
    \node [database, below of=backend, yshift=-1.0cm, xshift=-2.5cm] (neo4j) {\textbf{Neo4j Graph DB}\\\faProjectDiagram\ Bolt: 7687, HTTP: 7474\\1.128 Nodes, 1.532 Rel};
    \node [database, below of=backend, yshift=-1.0cm, xshift=2.5cm] (qdrant) {\textbf{Qdrant Vector DB}\\\faCloud\ Port 6333 / Cloud\\Chỉ mục Rules \& Facts};

    \draw [arrow, <->] (frontend) -- node[above] {HTTP / REST \& SSE} (backend);
    \draw [arrow, <->] (backend) -- node[left, near end] {Cypher Bolt} (neo4j);
    \draw [arrow, <->] (backend) -- node[right, near end] {gRPC / REST} (qdrant);
\end{tikzpicture}
\caption{Sơ đồ kiến trúc Microservices Docker của hệ thống AlphaGeometry-IMO.}
