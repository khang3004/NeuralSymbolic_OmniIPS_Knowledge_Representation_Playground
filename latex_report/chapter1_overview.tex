...
\begin{tikzpicture}[node distance=1.8cm, auto]
    \node [base, fill=blue!15, text width=3.8cm] (input) {\textbf{Đề bài Hình học}\\(Ngôn ngữ tự nhiên / Vị từ)};
    \node [process, fill=green!15, right of=input, xshift=3.5cm, text width=4.5cm] (dd) {\textbf{Symbolic Engine (DD+AR)}\\Suy diễn tiến + Đại số hóa};
    \node [decision, below of=dd, yshift=-0.8cm] (check) {Mục tiêu đạt được?};
    \node [base, fill=teal!20, right of=check, xshift=3.5cm, text width=3.5cm] (success) {\textbf{Chứng minh Thành công!}\\\faCheckCircle\ Xuất lời giải logic};
    \node [agent, fill=purple!20, left of=check, xshift=-3.5cm, text width=4.2cm] (neural) {\textbf{Neural Model (LLM)}\\Đề xuất hình phụ (Auxiliary)};

    \draw [arrow] (input) -- (dd);
    \draw [arrow] (dd) -- (check);
    \draw [arrow] (check) -- node[above] {Có} (success);
    \draw [arrow] (check) -- node[left] {Không (Bế tắc)} (neural);
    \draw [arrow] (neural) |- node[near start, left] {Thêm điểm/đường phụ} (dd);
\end{tikzpicture}
