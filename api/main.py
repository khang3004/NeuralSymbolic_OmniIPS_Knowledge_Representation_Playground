...
FALLBACK_GEOMETRY_RULES = [
    ...
    {"id": "geo_circumcenter_midpoint_perp_var_3arg", "name": "Circumcenter Midpoint Perpendicular Bisector (3-arg)",
     "inputs": ["Triangle(?A,?B,?C)", "Circumcircle(?O,?A,?B,?C)", "Midpoint(?M,?B,?C)"],
     "outputs": ["Perpendicular(?O?M,?B?C)", "Perpendicular(?M?O,?B?C)", "RightAngle(Angle(?O?M?B))"],
     "description": "The segment from circumcenter O to midpoint M of BC is perpendicular to BC."},
    ...
    {"id": "geo_square_intersection_coincident_var", "name": "Square Circumcircle Intersection Coincidence",
     "inputs": ["PointOnSegment(?M,?A,?B)", "Square(?A,?M,?C,?D)", "Square(?M,?B,?E,?F)"],
     "outputs": ["Equal(Point(?N),Point(?N_prime))", "Equal(?N,?N_prime)", "Coincident(?N,?N_prime)", "Point(?N)", "Equal(N,N)"],
     "description": "Intersection of square circumcircles coincides with AF and BC intersection."},
    {"id": "geo_square_definition_from_facts_var", "name": "Square Definition from Congruent Perpendicular Segments",
     "inputs": ["PointOnSegment(?M,?A,?B)", "Congruent(?A?M,?C?M)", "Perpendicular(?A?M,?C?M)"],
     "outputs": ["Square(?A,?M,?C,?D)", "Square(?M,?B,?E,?F)", "Point(?N)", "Equal(N,N)"],
     "description": "Forming square structures from perpendicular congruent segments."},
    {"id": "geo_right_triangle_angle_var", "name": "Right Triangle Right Angle Definition",
     "inputs": ["RightTriangle(?A,?B,?C)"],
     "outputs": ["RightAngle(Angle(?B?A?C))", "RightAngle(Angle(?C?A?B))", "RightAngle(Angle(?A))"],
     "description": "A right triangle ABC at vertex A has a right angle BAC."},
    {"id": "geo_triangle_inset_area_inequality_var", "name": "Triangle Inset Area Inequality",
     "inputs": ["Triangle(?A,?B,?C)", "PointOnSegment(?K,?B,?C)", "PointOnSegment(?L,?C,?A)", "PointOnSegment(?M,?A,?B)"],
     "outputs": ["LessEqual(Area(Triangle(?A,?M,?L)),Mul(Div(1,4),Area(Triangle(?A,?B,?C))))", "Triangle(?A,?M,?L)"],
    ...
    {"id": "geo_isogonal_altitude_circumcenter_var", "name": "Isogonal Property of Altitude and Circumcenter",
     "inputs": ["Triangle(?A,?B,?C)", "Foot(?H,?A,?B,?C)", "Circumcircle(?O,?A,?B,?C)"],
     "outputs": ["Equal(Angle(?B?A?H),Angle(?C?A?O))", "Equal(Angle(?H?A?B),Angle(?O?A?C))"],
     "description": "Altitude AH and circumcenter line AO are isogonal conjugates with respect to angle A."},
    {"id": "geo_isogonal_altitude_circumcenter_var_2", "name": "Isogonal Property of Altitude and Circumcenter (Circumcenter variant)",
     "inputs": ["Triangle(?A,?B,?C)", "Foot(?H,?A,?B,?C)", "Circumcenter(?O,?A,?B,?C)"],
     "outputs": ["Equal(Angle(?B?A?H),Angle(?C?A?O))", "Equal(Angle(?H?A?B),Angle(?O?A?C))"],
     "description": "Altitude AH and circumcenter line AO are isogonal conjugates with respect to angle A."},
    {"id": "geo_isogonal_altitude_circumcenter_var_3", "name": "Isogonal Property of Altitude and Circumcenter (Congruent radii variant)",
     "inputs": ["Triangle(?A,?B,?C)", "Foot(?H,?A,?B,?C)", "Congruent(?A?O,?B?O)"],
     "outputs": ["Equal(Angle(?B?A?H),Angle(?C?A?O))", "Equal(Angle(?H?A?B),Angle(?O?A?C))"],
     "description": "Altitude AH and circumcenter line AO are isogonal conjugates with respect to angle A."},
    {"id": "geo_right_angle_is_right_triangle_var", "name": "Right Angle Definition of Right Triangle",
     "inputs": ["Triangle(?A,?B,?C)", "RightAngle(Angle(?B?A?C))"],
     "outputs": ["RightTriangle(?A,?B,?C)"],
     "description": "A triangle with right angle BAC is a right triangle ABC."},
    {"id": "geo_triangle_inset_points_not_collinear_var", "name": "Triangle Inset Points Form Non-Collinear Triangle",
     "inputs": ["Triangle(?A,?B,?C)", "PointOnSegment(?L,?C,?A)", "PointOnSegment(?M,?A,?B)"],
     "outputs": ["NotCollinear(?A,?M,?L)", "Triangle(?A,?M,?L)"],
     "description": "Points on non-parallel sides of a triangle form a non-collinear triangle."},
